"""Run a fixed list of OOD prompts through a checkpoint and print results."""
import sys
import torch
...
PROMPTS = [
    ('factorial 7',        'compute 7 factorial and store it',                   'mem',     120*42),  # 5040
...
def expected(label: str, val):
    ...
    if label == 'mem10':  return [0, 1, 1, 2, 3, 5, 8, 13]   # first 8 of fib 10
    if label == 'mem20':  return [20, 19, 18, 17, 16, 15, 14, 13]  # first 8 of countdown 20
    if label == 'dst8':   return None


def main():
    ckpt = sys.argv[1] if len(sys.argv) > 1 else 'reflex_grounded.pt'
    ...
    for tag, prompt, kind, val in PROMPTS:
        ...
        if kind == 'dst8':
            ...
            ok = got[:len(exp)] == exp[:len(exp)] if kind != 'mem' else got[0] == val
            result = f'mem={got[:8]}'
