...
ASM_SYS_PROMPT = (
    ...
    "Data memory at 0x5000. Store the result there. Display buffer at "
    "0x6000 (ASCII, one byte per 4-byte word). Use EBREAK to halt.\n\n"
    ...
    "IMPORTANT: memory ops use `offset(base_reg)` syntax only — e.g. "
    "`sw x7, 0(x10)`. To store at absolute address 0x5000, first load "
    "the base with `lui x10, 5` (makes x10 = 0x5000), then `sw x7, 0(x10)`. "
    "Never write `sw x7, 0x5000` — that's invalid.\n\n"
